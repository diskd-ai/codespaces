...
from scripts.lang.pascal.imports import BoundPascalLanguage
...
""".lstrip(),
    ...
    def test_unreadable_pascal_source_surfaces_resolver_diagnostic(self) -> None:
        """/* REQ-PAS-019: Pascal resolver reads surface failures instead of silently publishing partial facts. */"""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory).resolve()
            missing_source = root / "Missing.pas"
            stderr = io.StringIO()

            with redirect_stderr(stderr):
                BoundPascalLanguage.create(
                    str(root),
                    {str(missing_source): "Missing"},
                    DiscoveryExclusions(frozenset(), frozenset()),
                )

        diagnostic = stderr.getvalue()
        self.assertIn("Cannot index Pascal declarations", diagnostic)
        self.assertIn(str(missing_source), diagnostic)
